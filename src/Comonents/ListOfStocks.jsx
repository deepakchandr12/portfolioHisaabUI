...
export default function ListOfStocks() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8082/stock/viewAll/${userLoggedIn.id}`
      );
  ...
  const handleModalClose = () => {
    ...
    fetchData(); // Refresh the stock list
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      selected.map((rowId) =>
        axios
          .delete(`http://localhost:8082/stock/delete/1/${rowId}`)
          .then((res) => {
            console.log(res);
            fetchData();
          })
  ...
  //   setEditStock(row); // Set the stock to be edited
  // };
                      ...
                      <TableCell align="right">
                        <IconButton
                          onClick={(event) => {
                            ...
                            setEditStock(row); // ✅ Now row is in scope
                          }}
                        >
                          <EditIcon stock={editStock} />
                        </IconButton>
                      </TableCell>
                    </TableRow>
                  );
